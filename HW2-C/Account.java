...
class Account {
    ...
    private LDLinkedList<Post> post;
    private LDLinkedList<Message> inbox;
    private LDLinkedList<Message> outbox;
    private boolean login_status = false;
    private LDLinkedList<Account> blockedid = new LDLinkedList<Account>();
    private LDLinkedList<Action> actionHistory = new LDLinkedList<Action>();
    ...
    public Account(int accountid, String username, String birthday, String location) {
        ...
        this.post = new LDLinkedList<Post>();
    }
    /**
    ...
    public void unlike(Post p)
    {
        ...
        boolean init = false;
        int index = 0;
        for(int i=0;i<p.getlikearr().size();i++)
        {
            if(p.getlikearr().get(i).getInteractionID() == this.accountid)
            {
                init=true;
                index=i;
                break;
    ...
    public void uncomment(Post p,int comment_number)
    {
        ...
        int index = 0;
        int count = 0;
        for(int i=0;i<p.getCommentarr().size();i++)
        {
            if(p.getCommentarr().get(i).getInteractionID() == this.accountid)
            {
    ...
    public void unfollow(Account target)
    {
        ...
        target.followers.removeLD(this);
        this.following.removeLD(target);
    ...
    public void unblock(Account target)
    {
        ...
        blockedid.removeLD(target);
        target.blockedid.removeLD(this);
    ...
    public void block(Account target) {
        ...
        followers.removeLD(target); // If this isnt following target,nothing will happen
        following.removeLD(target);
        target.followers.removeLD(this);
        target.following.removeLD(this);
        ...
        for (int i = 0; i < post.size(); i++) {
            ...
            if (post.get(i).getLikeCount() == 0) {
                ...
                for (int n = 0; n < post.get(i).getLikeCount(); n++) {
                    if (post.get(i).getlikearr().get(n) != null)
                        System.out.print(post.get(i).getlikearr().get(n).getInteractionName() + ",");
                }
                System.out.print("\n");
            }
            if (post.get(i).getCommentCount() == 0) {
                ...
                for (int n = 0; n < post.get(i).getCommentCount(); n++) {
                    if (post.get(i).getCommentarr().get(n) != null)
                        System.out.print(post.get(i).getCommentarr().get(n).getInteractionName() + ",");
                }
                System.out.println();
                for (int a1 = 0; a1 < post.get(i).getCommentCount(); a1++) {
                    if (post.get(i).getCommentarr().get(a1) != null)
                        System.out.println("Comment " + a1 + 1 + " :(" + 
                        post.get(i).getCommentarr().get(a1).getInteractionName()+ ") ==> " +
                        post.get(i).getCommentarr().get(a1).getContent());
                }
                System.out.println();
    ...
    public void getInbox() {
        ...
        System.out.println("Accessing the inbox of " + getUsername() + inbox.size());
    ...
    public LDLinkedList<Action> getActionHistory()
    {
